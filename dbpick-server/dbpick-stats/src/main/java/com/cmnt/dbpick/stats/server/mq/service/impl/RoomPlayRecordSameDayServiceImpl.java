...
@Slf4j
@Component
@SameDayHandlerEventType(SameDayEnum.SAME_DAY)
public class RoomPlayRecordSameDayServiceImpl implements RoomPlayRecordStatsService {
    ...
    public void execute(RoomPlayRecordMessage record) {
        ...
        log.info("根据直播记录统计直播数据 直播时间 one day={}~{}",recordStartTime,recordStopTime);
        ...
        dayList.forEach(
                dayStr -> {
                    RoomPlayRecordParams param = RoomPlayRecordParams.builder().roomNo(record.getRoomNo())
                            .stopTime(dayStr.getCurrentDayMax()).startTime(dayStr.getCurrentDayMin()).build();
